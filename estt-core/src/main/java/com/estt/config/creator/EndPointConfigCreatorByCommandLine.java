...
public class EndPointConfigCreatorByCommandLine {
	...
	/**
	 * This method is used to create configuration files under configuration file
	 * location
	 * 
	...
	public void createConfigurationFiles() throws Exception {
		...
		while (true) {
			...
			if ((StringUtils.isEmpty(methodType))
					|| (!StringUtils.isEmpty(methodType) && !methodTypes.contains(methodType))) {
				System.out.println("Provided method Type:[" + methodType
						+ "] not support for now, please enter again either GET, POST, PUT, DELETE end points to test.!!");
			...
			if (!"GET".equalsIgnoreCase(methodType)) {
				System.out
						.println("Please provide Request Body, if not giving next option please provide double enter:");
			...
			if ("n".equalsIgnoreCase(nextEP.trim())) {
				...
				ConfigFileCreatorUtil.writeFile(this.configFileLoc, "EndPointConfig.json",
