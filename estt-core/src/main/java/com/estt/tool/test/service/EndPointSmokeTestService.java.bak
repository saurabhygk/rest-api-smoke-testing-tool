...
public class EndPointSmokeTestService {
	...
	public void testEndPoints(String... arguments) throws Exception {
		...
		String statusFileName = arguments[0].concat(STATUS_FILE_NAME);
	...
	public void testEndPoints(List<EndPointConfig> endPoints, List<String> errorCodes, String statusFileDirPath, String userId)
		...
		for (EndPointConfig endPoint : endPoints) {
			redisHelper.putValueWithExpireTime(userId.concat("-STATUS"), 0, 15, TimeUnit.MINUTES);
			try {
				...
				try {
					...
					if (null == endPointResponse && !StringUtils.isEmpty(endPoint.getExpectedResponse())) {
						errorCount++;
						redisHelper.putValueWithExpireTime(userId.concat("-EC"), errorCount, 15, TimeUnit.MINUTES);
						...
						successCount++;
						redisHelper.putValueWithExpireTime(userId.concat("-SC"), successCount, 15, TimeUnit.MINUTES);
					} else {
		...
		redisHelper.putValueWithExpireTime(userId.concat("-STATUS"), 1, 15, TimeUnit.MINUTES);
	}
