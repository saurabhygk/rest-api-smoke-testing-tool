...
/**
 * This application will generate configuration file(s) for end point smoke test
 * tool * EndPointConfig.json * ErrorCodes.properties
 */
public class EndPointConfigCreator {
	// args[0] : to store created config file
	// args[1] : Optional : if user wants to create configuration file by reading
	// datafile (location of datafile with name datafile.txt)
	public static void main(String[] args) throws Exception {
		if (args.length == 1) {
			...
			System.out.println(
					"\t 2. Only provide first argument to create configuration file using command line options");
			return;
		}
	}
}
